...
df_isna = df_data.isna().apply(lambda x: get_pattern(x), axis=1).to_frame(name='pattern')
df_isna_pattern = df_isna['pattern'].value_counts().rename_axis('pattern').reset_index(name='counts')
```

```python
df_isna_pattern.iloc[:15]
```

```python
df_data_pattern = df_data.loc[df_isna[df_isna['pattern'].isin(['_EMPTY_' , 'PM2.5'])].index]
```

```python tags=[]
n_stations = len(df_data_pattern.groupby("station").size())
n_cols = len(cols_to_impute)
```

```python tags=[]
# fig = plt.figure(figsize=(10 * n_stations, 3 * n_cols))
# for i_station, (station, df) in enumerate(df_data_pattern.groupby("station")):
#     df_station = df_data_pattern.loc[station]
...
```python
cols_to_impute = df_data_pattern.columns[~df_data_pattern.isna().any()]
print('cols without nan: ',  cols_to_impute)

generator_holes = missing_patterns.EmpiricalHoleGenerator(n_splits=2, groups=["station"], ratio_masked=ratio_masked, subset=cols_to_impute).fit(df_data)
```
...
results = comparison.compare(df_data_pattern, metrics=mtrs)
results
```

```python
fig = plt.figure(figsize=(24, 4))
plot.multibar(results.loc["mse"], decimals=1)
plt.ylabel("mae")
plt.show()
```
