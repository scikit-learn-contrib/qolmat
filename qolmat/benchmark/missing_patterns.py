...
class BernoulliHoleGenerator(_HoleGenerator):
    """This class implements a way to generate holes in a dataframe.
    The holes are generated by selecting which feature/variable is masked.
    The selection is based on Bernoulli distribution.
    ...
    def generate_mask(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        ----------
        X : pd.DataFrame
            Initial dataframe with a missing pattern.
        """
        ratio_nan = X.isna().sum().sum() / X.size
        np_mask = np.random.choice(
            [False, True],
            size=(len(X.index), len(X.columns)),
            p=[1.0 - (self.ratio_masked + ratio_nan), self.ratio_masked + ratio_nan],
        )
        df_mask = pd.DataFrame(np_mask, index=X.index, columns=X.columns)
        df_mask[X.isna()] = False
