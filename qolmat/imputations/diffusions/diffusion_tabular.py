...
class TabDDPM(DDPM):
    def __init__(self, dim_input: int, num_noise_steps: int = 50, beta_start: float = 1e-4, beta_end: float = 0.02, lr: float = 0.01, ratio_masked: float = 0.1, dim_embedding: int = 128, num_blocks: int = 1, p_dropout: float = 0.1):
    ...
    def fit(self, x: pd.DataFrame, epochs=10, batch_size=100, x_valid=None, x_valid_mask=None, print_valid=True):
        self.eps_model = AutoEncoder(self.num_noise_steps, self.dim_input, self.dim_embedding, self.num_blocks, self.p_dropout).to(self.device)
        ...
        for epoch in range(epochs):
            ...
            if print_valid and (epoch) % print_step == 0:
                print(f"Epoch {epoch}: epoch_loss={self.summary['epoch_loss'][epoch]} in {round(time_duration, 3)}")
