...
from qolmat.imputations.diffusions.diffusion_tabular import AutoEncoder
...
class TabDDPMTS(DDPM):
    ...
    def fit(
        ...
        if x_valid is not None:
            ...
            x_processed_valid, x_mask_valid = self._process_data(
                x_valid, x_valid_mask, size_window=self.size_window
            )
        ...
        for epoch in range(epochs):
            ...
            if print_valid and (epoch) % print_step == 0:
                print(
                    f"Epoch {epoch}: epoch_loss={self.summary['epoch_loss'][epoch]} in {round(time_duration, 3)} s"
                )
