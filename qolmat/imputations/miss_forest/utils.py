...
def masked_euclidean_distances(
    X, Y=None, squared=False, missing_values="NaN", copy=True
):
    ...
    * John K. Dixon, "Pattern Recognition with Partly Missing Data",
    ...
    X, Y = check_pairwise_arrays(
        X, Y, accept_sparse=False, force_all_finite=False, copy=copy
    )
    ...
    if np.any(mask_X.sum(axis=1) == X.shape[1]) or (
        Y is not X and np.any(mask_YT.sum(axis=0) == Y.shape[1])
    ):
