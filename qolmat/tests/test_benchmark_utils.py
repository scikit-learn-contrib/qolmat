...
def test_get_search_space() -> None:
    ...
    search_params = {"ImputeKNN": {"k": {"min": 2, "max": 10, "type": "Integer"}}}
...
def test_choice_with_mask() -> None:
    df = pd.DataFrame(
        [[1, 2, 3], [1, 2, np.nan], [1, 2, 3], [1, 2, 3], [1, 2, 3], [np.nan, 2, 3]],
    ...
    assert_frame_equal(
        utils.choice_with_mask(df, mask, ratio, random_state=random_state), result
    )
...
def test_mean_squared_error() -> None:
    ...
    assert_series_equal(
        utils.mean_squared_error(df1, df2, squared=True, columnwise_evaluation=True),
        pd.Series([94, 58, 25], index=["var1", "var2", "var3"]),
    )
    assert_series_equal(
        utils.mean_squared_error(df1, df2, squared=False, columnwise_evaluation=True),
        pd.Series([9.695360, 7.615773, 5], index=["var1", "var2", "var3"]),
    )
    assert (
        utils.mean_squared_error(df1, df2, squared=True, columnwise_evaluation=False)
        == 177
    )
    assert (
        round(
...
def test_weighted_mean_absolute_percentage_error() -> None:
    ...
    assert_series_equal(
        ...
        pd.Series([0.727273, 0.384615, 0.233333], index=["var1", "var2", "var3"]),
    )
    assert (
        round(
            utils.weighted_mean_absolute_percentage_error(
...
def test_kl_divergence() -> None:
    ...
    assert (
        round(utils.kl_divergence(df1, df2, columnwise_evaluation=False), 4) == 14.0112
    )
