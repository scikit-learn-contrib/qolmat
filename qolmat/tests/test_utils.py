...
def test_check_dfs_columns():
    ...
    with pytest.raises(
        ValueError,
        match=r".*The columns of the two dataframes do not match*"):
            utils._check_dfs_columns(df0_c0, df0_c2)

def test_mean_squared_error():
    assert utils.mean_squared_error(df0, df1) == 1.
    assert utils.mean_squared_error(df0, df2) == 4.

def test_root_mean_squared_error():
    assert utils.root_mean_squared_error(df0, df1) == 1.
    assert utils.root_mean_squared_error(df0, df2) == 2.

def test_mean_absolute_error():
    assert utils.mean_absolute_error(df0, df1) == 1.
    assert utils.mean_absolute_error(df0, df2) == 2.
...
def test_wasser_distance_error():
    ...
    with pytest.raises(
        ValueError,
        match=r".*The column a contains nan in one of the dataframe*"):
