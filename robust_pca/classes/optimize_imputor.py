...
class get_objective:
    def __init__(
        self, imputor: RPCA, imputor_eval: EvaluateImputor, space, scoring, exp  # rmse
    ):
    ...
    def fit_transform_and_evaluate(self):
        ...
        def obj_func(**all_params):
            ...
            if self.exp:
                all_params = dict(
                    map(lambda x: (x[0], np.exp(x[1] * np.log(10))), all_params.items())
                )
...
def rpca_optimizer(
    ...
    best_params = {
        space[index].name: result["x"][index] for index in range(len(result["x"]))
    }
